...
let state = { gps:null, destino:null, lote:"", vins:[], obs:"Sin observaciones",documento:null,documentos:[]};
...
function renderStep(){
  ...
  if(step===6){
    ...
    $("stepContent").innerHTML=`
      ...
      ${state.documentos && state.documentos.length ? `<p class="small ok">${state.documentos.length} documento(s) seleccionado(s).</p>` : `<p class="small">Sin documento adjunto.</p>`}
...
function handleDocumentos(event){
  ...
  state.documentos = files.map((file, index) => ({
  ...
  const list = $("docList");
  if(list){
    list.innerHTML = state.documentos.map((d,i)=>`
      <div class="docItem">${i+1}. ${d.name}</div>
    `).join("");
  }

  const nameBox = $("docName");
  if(nameBox){
    nameBox.innerText = state.documentos.length + " documento(s) seleccionado(s)";
  }
}
