...
let state = { gps:null, destino:null, lote:"", vins:[], obs:"Sin observaciones",documento:null};
  ...
  if(step===5){
    ...
    $("stepContent").innerHTML=`
      ...
      <div class="docBox">
        ...
        <input class="fileInput" id="docFile" type="file" accept="image/*,.pdf" capture="environment" onchange="handleDocumento(event)">
        <div id="docName" class="docName">${doc ? doc.name : "Sin documento adjunto"}</div>
        <img id="docPreview" class="docPreview" ${doc && doc.preview ? `src="${doc.preview}" style="display:block"` : ""}>
      </div>
      <div class="stepActions">
        <button class="btn back" onclick="previousStep()">Volver</button>
        <button class="btn" onclick="step=6;renderStep()">Continuar</button>
      </div>`;
  }

  if(step===6){
    $("stepTitle").innerText="Paso 6";
    const msg = buildMessage(false);
    $("stepContent").innerHTML=`
      <div class="summary">${escapeHtml(msg)}</div>
      ${state.documento ? `<p class="small ok">Documento seleccionado: ${state.documento.name}</p>` : `<p class="small">Sin documento adjunto.</p>`}
...
${unidades}
Documento: ${state.documento ? state.documento.name : "Sin documento"}\nObservación general: ${state.obs || "Sin observaciones"}`;
...
function handleDocumento(event){
  ...
  state.documento = {name:file.name || "documento", type:file.type || "", size:file.size || 0, preview:""};
  const nameBox = $("docName");
  if(nameBox) nameBox.innerText = state.documento.name;
  if(file.type && file.type.startsWith("image/")){
    const reader = new FileReader();
    reader.onload = e => {
      state.documento.preview = e.target.result;
      const img = $("docPreview");
      if(img){ img.src = state.documento.preview; img.style.display = "block"; }
    };
...
function sendWhatsapp(){
  ...
  state={gps:null,destino:null,lote:"",vins:[],obs:"Sin observaciones",documento:null};
  step=1;
